...
class SpatialEmbeddings(nn.Module):
    ...
    def forward(self, bbox):
        ...
        # w_position_embeddings =total_noans =  0 self.w_position_embeddings(bbox[:, :, 2] - bbox[:, :, 0])  # TODO Remove width and height to test how much important are they.
...
class VisualEmbeddings(nn.Module):
    ...
    def visualize_att_maps(self, attention_maps, images):
        # Visualize attention maps
        print("Visualize an element:")
        num_layers = len(attention_maps)
        num_heads = attention_maps[0].shape[1]
        
        # Overlay attention maps onto input image
        ...
        for head in range(num_heads):
            fig = plt.figure()
            #fig, axes = fig.subplots(1,1, figsize=(20, 20))
            axes = fig.add_subplot(1, 1, 1)
            att_map = attention_maps[0][0][head].cpu().detach().numpy()
            
            axes.imshow(images[0])
            axes.imshow(att_map, cmap='hot', alpha=0.5)  # Overlay attention map
            axes.axis('off')
            axes.set_title(f"Layer {0+1}, Head {head+1}")
    ...
    def forward(self, images, page_idx_mask=None):
        ...
        vis_embeddings = self.image_model(inputs.pixel_values.to(self.image_model.device), output_attentions=True)
       
        att_maps = vis_embeddings.attentions
        #self.visualize_att_maps(attention_maps= att_maps, images=images)
