...
def train_epoch(data_loader, model, optimizer, lr_scheduler, evaluator, logger, **kwargs):
    ...
    for batch_idx, batch in enumerate(tqdm(data_loader)):
        ...
        print("OUTPUTS IN TRAIN:", outputs.keys() )
        ...
        batch_noans_acc = np.mean(metric['noans_accuracy'])
        noans_samples = len(metric['noans_accuracy'])
        with_ans_acc = np.mean(metric['with_ans_accuracy'])
        total_noans += noans_samples
        ...
        log_dict = {
            ...
            'Train/Batch noans Accuracy': batch_noans_acc,
            'Train/noans samples': total_noans,
            'Trian/with ans accuracy': with_ans_acc,
...
def train(model, **kwargs):
    ...
    val_data_loader   = DataLoader(val_dataset, batch_size=config['batch_size'], shuffle=True, collate_fn=singlepage_docvqa_collate_fn)
    ...
    if kwargs.get('eval_start', False):
        ...
        accuracy, anls, noans_accuracy,noans_samples,with_ans_accuracy, ret_prec, _, _ = evaluate(val_data_loader, model, evaluator, return_scores_by_sample=False, return_pred_answers=False, **kwargs)
        is_updated = evaluator.update_global_metrics(accuracy, anls, -1)
        logger.log_val_metrics(accuracy, anls, noans_accuracy, noans_samples, with_ans_accuracy, ret_prec, update_best=is_updated)
